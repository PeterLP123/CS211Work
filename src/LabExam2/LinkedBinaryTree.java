...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
    ...
    public void swapChildren(Position<E> p) throws IllegalArgumentException{
        ...
        if(node.getLeft() == null || node.getRight() == null){
            throw new IllegalArgumentException("Cannot swap children of a node with only one child");
        }
    ...
    public void printChildren(Position<E> p) throws IllegalArgumentException{
        Node<E> node = validate(p);
        System.out.println("The left child of " + node.getElement() + " is: " + node.getLeft().getElement());
        System.out.println("The right child of: " + node.getElement() + " is: " + node.getRight().getElement());
    }

    public void swap(Position<E> p, Position<E> q) {
        Node<E> node1 = validate(p);
        Node<E> node2 = validate(q);
        if(node1.getParent() == node2 || node2.getParent() == node1){
            throw new IllegalArgumentException("Cannot swap parent and child");
        }
        else{
            Node<E> p1 = node1.getParent();
            Node<E> p2 = node2.getParent();
            if(p1 == p2){
                swapChildren(p1);
                return;
            }
    ...
    public boolean isProperTree(){
        ...
        Iterable<Position<E>> positions = inorder();
        for(Position<E> p : positions){
            if(!isExternal(p)){
